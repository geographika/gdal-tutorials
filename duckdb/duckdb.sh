...
gdal vector convert --format parquet POI_FR_H32.parquet \
...
gdal vector pipeline \
    ...
    --oo SQL="select h3_id, ST_GeomFromText(h3_cell_to_boundary_wkt(h3_id)) geometry, count(*) POI_count \
        ...
        where addresses[1].country='FR' and bbox.xmin between -5 and 9 and bbox.ymin between 41 and 51) group by h3_id" \
    ! edit --crs epsg:4326 \
    ! write --format parquet POI_FR_H33.parquet

# set-geom-type  in 3.12 only
#     ! set-geom-type --geometry-type POINT \

gdal vector info POI_FR_H32.parquet

ogrinfo POI_FR_H32.parquet